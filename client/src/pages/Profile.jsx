  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        fileUploadError?
        (<span className="text-red-700">Error Image Upload(should be less than 2mb)</span>)  
        :
      ...
      <p className="text-red-700 mt-5">
        {showListingError ? "Error showing listings":null}
      </p>
      {userListings && userListings.length > 0 && 
        <div className="flex flex-col gap-4">
          <h1 className="text-center text-2xl py-3 rounded-lg font-semibold bg-indigo-800 text-white">User Listings</h1>
          {
            userListings.map(listing=>(
              <div className="border rounded-lg p-3 flex justify-between items-center gap-4" key={listing._id}>
                <Link to={`/listing/${listing._id}`}>
                  <img src={listing.imageUrls} alt="listings" className="h-16 w-16 object-contain" />
                </Link>
                <Link to={`/listing/${listing._id}`} className="text-slate-700 font-semibold flex-1 hover:underline truncate">
                 <p >{listing.name}</p>
                </Link>
               <div className="flex flex-col items-center">
                <button onClick={()=>handleListingDelete(listing._id)} type="button" className="text-red-700 uppercase">Delete</button>
